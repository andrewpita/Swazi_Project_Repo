...
#prior parameters for beta priors
alpha = beta = 1
...
#function that takes as input a column of the 
#gibbs sampler iterations (each column corresponds
...
#data 
sim_data_gibbs_samp = function(seed, N = 10000, a.prior =alpha, b.prior = beta) {
  ...
  Nh.sample = rmultinom(1, size = true.N.Nh, prob = 
                          c(true.p.srv.Nh*(1 - true.p.uid.Nh)*(1 - true.p.rnb.Nh), #only in survey
                            ...
                            (1- true.p.srv.Nh)*(1 - true.p.uid.Nh)* (1 - true.p.rnb.Nh)#in none
                          ))
...
write.csv(avgsum,paste(alpha,"summary.csv",sep = ""))
